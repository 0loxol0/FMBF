...
def tokenz():
	...
	try:
		...
		os.system('clear')
		logo()
		print(" %s[*] Author     : Ahmed Husen Al dukali XZ."%(N))
		print(" [*] Github     : https://github.com/AhmedAlzwage")
		print(" [*] ---------------------------------------------")
		print(" [*] Bergabung  : %s"%(tgl))
		print(" [*] Status     : %sHacker Libya%s"%(H,N))
		print(" [*] ---------------------------------------------")
		print(" [*] IP         : %s"%(IP))
		token = raw_input('\n [?] token : ')
		try:
...
def menu():
    ...
    logo()
    print(" %s[*] Author    : AhmedAlzwage."%(N))
    print(" [*] Github    : https://github.com/Libya pro")
    print(" [*] --------------------------------------------")
    print(" [*] Bergabung : %s"%(tgl))
    print(" [*] Status    : %sHacker Libya%s"%(H,N))
    print(" [*] --------------------------------------------")
...
def followers():
	...
	idt = raw_input(" [*] masukan id atau username : ")
	try:
		for i in requests.get("https://graph.facebook.com/%s/subscribers?limit=90000&access_token=%s"%(idt, token)).json()["data"]:
			uid = i["id"]
			nama = i["name"]
			id.append(uid+"<=>"+nama)
	except KeyError:
